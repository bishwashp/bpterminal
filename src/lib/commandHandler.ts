			...
			switch (target) {
				case 'about':
					return [
						...
						'Skills: Svelte, TypeScript, Linux...'
					];
				case 'projects':
				...
				case 'contact':
					return [
						'# Contact',
						'Use the form on /contact or email directly at placeholder@example.com'
					];
				default:
...
commands['list-fonts'] = {
	...
	action: () => {
		return [
			'Available fonts:',
			...availableFonts.map(
				(f, i) => `  [${i}] ${f.split(',')[0].replace(/['"]/g, '')}` // Show index and clean name
			)
		];
	}
};
...
export function getPathCompletions(): string[] {
