...
function getInitialTheme(): ThemeSettings {
    ...
    if (storedSettings) {
        try {
            ...
            return {
                primaryColor: parsed.primaryColor && /^#[0-9a-fA-F]{6}$/.test(parsed.primaryColor) ? parsed.primaryColor : defaultTheme.primaryColor,
                secondaryColor: parsed.secondaryColor && /^#[0-9a-fA-F]{6}$/.test(parsed.secondaryColor) ? parsed.secondaryColor : defaultTheme.secondaryColor,
                fontFamily: availableFonts.includes(parsed.fontFamily) ? parsed.fontFamily : defaultTheme.fontFamily,
            };
        } catch (e) {
...
export const theme = {
    ...
    setColors: (primary: string, secondary: string) => {
         ...
         if (/^#[0-9a-fA-F]{6}$/.test(primary) && /^#[0-9a-fA-F]{6}$/.test(secondary)) {
             themeSettingsStore.update(s => ({ ...s, primaryColor: primary, secondaryColor: secondary }));
             return true; // Indicate success
         } else {
