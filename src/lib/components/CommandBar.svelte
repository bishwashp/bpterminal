...
<script lang="ts">
    ...
    async function handleCommand() {
        ...
        if (resultString && resultString !== '__CLEAR__') { // Don't show output for clear or void results
             outputType = (resultString.toLowerCase().startsWith('error:') || resultString.toLowerCase().includes('not found') || resultString.toLowerCase().includes('cannot access') || resultString.toLowerCase().includes('no such file')) 
                                ? 'error' 
                                : 'output';
...
<div class="command-bar-container">
    {#if showOutput}
        <div class="command-output" class:error={outputType === 'error'} transition:fly={{ y: 10, duration: 300 }}>
            <pre>{outputText}</pre>
        </div>
    {/if}
