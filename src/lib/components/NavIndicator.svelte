...
<div class="nav-indicator-container" >
    ...
    {#if showMenu}
        <nav>
            ...
            on:keydown={(e: KeyboardEvent) => { if (e.key === 'Escape') closeMenu(); }}
            on:focusout={(e: FocusEvent) => {
                if (!e.currentTarget || !(e.currentTarget as HTMLElement).contains(e.relatedTarget as Node)) {
                    closeMenu();
                }
            }}
            tabindex="-1" /* Make nav focusable */
            <ul>
                {#each pages as navItem, i}
                    <li role="menuitem">
                        <a 
                            href={navItem.path} 
                            aria-current={$page.url.pathname === navItem.path ? 'page' : undefined}
                            on:blur={(e: FocusEvent) => { 
                                if (i === pages.length - 1 && !(e.currentTarget as HTMLElement).closest('.nav-menu')?.contains(e.relatedTarget as Node)) {
                                    closeMenu();
                                }
                            }}
                        >
