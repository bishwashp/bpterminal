<script lang="ts">
    ...
    import { executeCommand, getAvailableCommands, getPathCompletions, availableFonts } from '../lib/commandHandler';
	...
	onMount(() => {
        ...
        output = welcomeMessages.map((msg, index) => ({
            id: lineIdCounter++,
            type: 'output',
            text: msg.text,
            useTypewriter: true,
	...
	async function handleCommand() {
        ...
        if (resultString === '__CLEAR__') {
            ...
            const resultType = (resultString.toLowerCase().startsWith('error:') || resultString.toLowerCase().includes('not found') || resultString.toLowerCase().includes('cannot access') || resultString.toLowerCase().includes('no such file')) 
                                ? 'error' 
    ...
    function handleKeyDown(event: KeyboardEvent) {
        ...
        if ((event.key === 'Tab' || (event.key === 'ArrowRight' && inputElement && inputElement.selectionStart === commandInput.length)) && suggestion) {
    ...
    function updateSuggestion() {
        ...
        if (!currentInput || commandInput.endsWith(' ')) { // Don't suggest if empty or ending with space
            suggestion = '';
            return;
        }
        ...
        if (isTypingCommand) {
            ...
            if (commandName === 'cd' || commandName === 'ls' || commandName === 'cat') {
                 candidates = getPathCompletions($page.url.pathname); // Use path helper
        ...
        const firstSuggestion = potentialCompletions.find(comp => comp.startsWith(lastPart) && comp !== lastPart);
		...
		{#each output as lineItem (lineItem.id)}
            ...
            {:else}
                ...
                <p 
                    ...
                    class:error={lineItem.type === 'error'} 
                    class:input-line={lineItem.type === 'input'}
                >{lineItem.text || '&nbsp;'}</p> <!-- Display text directly, use nbsp if empty -->
            {/if}
		{/each}
	</div>
	<div class="input-area">
        ...
        <div class="input-wrapper">
            ...
            {#if suggestion && suggestion.startsWith(commandInput) && suggestion.length > commandInput.length}
                <span class="suggestion">{commandInput}<span class="suggestion-suffix">{suggestion.slice(commandInput.length)}</span></span>
            {/if}
        </div>
	</div>
</div>
...
<style>
    ...
    /* Hide elements intended for typewriter until ready */
    p[hidden] {
        display: none; /* Use display: none for initial hide, then remove attribute */
    }
